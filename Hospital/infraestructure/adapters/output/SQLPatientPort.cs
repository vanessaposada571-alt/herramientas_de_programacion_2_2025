...
namespace Hospital.infraestructure.adapters.output
{
    public class SQLPatientPort : Patient_port
    {
        ...
        public Patient FindById_patient(Patient patient)
        {
            ...
            const string sql = @"
                SELECT p.PatientId, p.PolicyNumber,
            ...
            res.Name1 = new Person { Name = rd["Name"]?.ToString() ?? "" };
            res.Gender1 = new Person { Gender = rd["Gender"] != DBNull.Value && Convert.ToBoolean(rd["Gender"]) };
            var emailStr = rd["Email"]?.ToString();
            if (!string.IsNullOrWhiteSpace(emailStr)) res.Email1 = new Person { Email = new MailAddress(emailStr) };
            return res;
        }

        public Patient FindByContact(Patient patient)
        {
            // Buscar paciente por contacto: usar tabla Contact
            const string sql = @"
        ...
        public Patient FindByIdSure(Patient patient)
        {
            ...
            cn.Open();
            using var cmd = new SqlCommand(sql, cn);
            cmd.Parameters.AddWithValue("@miId", patient.IdSure?.IdSure ?? 0);
            using var rd = cmd.ExecuteReader(CommandBehavior.SingleRow);
            return rd.Read() ? new Patient { Id_patient = rd["PatientId"]?.ToString() ?? "" } : null;
        }
        ...
        {
            // Guardado mínimo: insertar Person y Patient (no Contact/Insurance completo)
            ...
            try
            {
                ...
                cmd1.Parameters.AddWithValue("@name", patient.Name1?.Name ?? "");
                cmd1.Parameters.AddWithValue("@idNumber", patient.Name1?.Id ?? 0L);
                cmd1.Parameters.AddWithValue("@email", patient.Email1?.Email?.Address ?? "");
                cmd1.Parameters.AddWithValue("@cellphone", patient.Cellphone1?.Cellphone ?? 0L);
                cmd1.Parameters.AddWithValue("@birthDate", (object)(patient.Birth == default ? DBNull.Value : patient.Birth));
                cmd1.Parameters.AddWithValue("@gender", patient.Gender1?.Gender == true ? 1 : 0);
                cmd1.Parameters.AddWithValue("@direction", patient.Direction1?.Direction ?? "");
                ...
                cmd2.Parameters.AddWithValue("@policyNumber", patient.PolicyNumber ?? "");
        ...
        public void Update(Patient p)
        {
            ...
            using var cn = GetConn();
            cn.Open();
            using var cmd = new SqlCommand(updatePerson, cn);
            cmd.Parameters.AddWithValue("@name", p.Name1?.Name ?? "");
            cmd.Parameters.AddWithValue("@email", p.Email1?.Email?.Address ?? "");
            cmd.Parameters.AddWithValue("@cellphone", p.Cellphone1?.Cellphone ?? 0L);
            cmd.Parameters.AddWithValue("@birthDate", (object)(p.Birth == default ? DBNull.Value : p.Birth));
            cmd.Parameters.AddWithValue("@gender", p.Gender1?.Gender == true ? 1 : 0);
            cmd.Parameters.AddWithValue("@direction", p.Direction1?.Direction ?? "");
        ...
        {
            // Implementación básica: eliminar paciente por Id
